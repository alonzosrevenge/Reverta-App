...
import { ArrowLeft, ArrowRight, Sunset, Play, Pause } from "lucide-react";
...
const MAGHRIB_STEPS = [
  ...
  // Second Rakah
  {
    ...
    description:
      "Stand for the second rak'ah and recite Al-Fātiḥa again. In the first two rak'ahs of Maghrib, both Al-Fātiḥa and an additional Surah are recited.",
  ...
  // Third Rakah
  {
    arabic:
      "Bismillāhi r-raḥmāni r-raḥīm. Al-ḥamdu lillāhi rabbi l-'ālamīn. Ar-raḥmāni r-raḥīm. Māliki yawmi d-dīn. Iyyāka na'budu wa iyyāka nasta'īn. Ihdinā ṣ-ṣirāṭa l-mustaqīm. Ṣirāṭa lladhīna an'amta 'alayhim ghayri l-maghḍūbi 'alayhim wa lā ḍ-ḍāllīn.",
    english: "Recite Al-Fātiḥa for the third and final rak'ah.",
    description:
      "Stand for the third rak'ah and recite Al-Fātiḥa. In the third rak'ah of Maghrib, only Al-Fātiḥa is recited (no additional Surah).",
    step: "Al-Fātiḥa (3rd Rakah)",
  },
  {
    arabic: "Allāhu Akbar",
    english: "Allāhu Akbar",
    description: "Move into Rukū' for the third and final rak'ah.",
    step: "Rukū' (3rd Rakah)",
  },
  {
    arabic: "Subḥāna rabbiya l-'aẓīm",
  ...
  },
  {
    ...
    english:
      "All compliments, prayers and pure words are due to Allah. Peace be upon you, O Prophet, and Allah's mercy and blessings. Peace be upon us and upon all righteous servants of Allah. I bear witness that none has the right to be worshipped except Allah, and I bear witness that Muhammad is His servant and messenger.",
    description:
      "Sit calmly and recite the complete Tashahhud after completing all three rak'ahs. Point your index finger during the testimony of faith.",
  ...
  },
  {
    ...
    description:
      "Turn your head to the RIGHT and say: 'As-salāmu 'alaykum wa raḥmatullāh' (Peace and mercy of Allah be upon you). Then turn your head to the LEFT and repeat the same greeting. This completes your Maghrib prayer.",
    step: "Tasleem (Final Salutations)",
  },
];

function MaghribIntro({ onBegin }: { onBegin: () => void }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden animate-fade-in">
      {/* Sunset gradient background effect */}
      ...
      <div className="text-center max-w-3xl mx-auto">
        <div className="mb-8 animate-scale-in">
          <div className="w-32 h-32 mx-auto rounded-3xl flex items-center justify-center shadow-2xl relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-br from-purple-500 via-pink-600 to-rose-700 animate-pulse"></div>
            <div className="absolute inset-[2px] bg-slate-900 rounded-3xl flex items-center justify-center">
              <div
                className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-br from-purple-400 to-pink-500"
        ...
        <div className="mb-8 animate-slide-up">
          <h1 className="text-6xl md:text-7xl font-black mb-4 leading-tight">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-500 to-rose-600">
              Maghrib
            </span>
          </h1>
          <p className="text-xl md:text-2xl text-slate-300 font-light mb-2">
            Sunset Prayer • 3 Rak'ahs
          </p>
          <p className="text-slate-400 text-sm md:text-base">
            The blessed sunset prayer as day transitions to night
          </p>
        </div>
        ...
        >
          <button
            onClick={onBegin}
            className="group relative px-8 py-4 bg-gradient-to-r from-purple-500 via-pink-600 to-rose-700 rounded-2xl font-semibold text-white text-lg shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105"
          >
            <span className="relative z-10 flex items-center gap-3">
              Begin Maghrib Prayer
...
export default function MaghribPage() {
  ...
  if (showIntro) {
    return <MaghribIntro onBegin={() => setShowIntro(false)} />;
  }

  const current = MAGHRIB_STEPS[currentStep];
  const progress = ((currentStep + 1) / MAGHRIB_STEPS.length) * 100;

  const handleNext = () => {
    if (currentStep < MAGHRIB_STEPS.length - 1) {
  ...
  return (
    <div className="min-h-screen flex flex-col p-6 relative overflow-hidden">
      {/* Sunset gradient background effect */}
      <div className="absolute inset-0 bg-gradient-to-b from-purple-900/10 via-pink-900/5 to-transparent pointer-events-none"></div>

      {/* Progress bar */}
      <div className="fixed top-0 left-0 w-full h-1 bg-slate-800/50 z-30">
        <div
          className="h-full bg-gradient-to-r from-purple-500 via-pink-600 to-rose-700 transition-all duration-500 ease-out"
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">
              Step {currentStep + 1} of {MAGHRIB_STEPS.length}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm rounded-3xl p-6 border border-slate-700/50">
              ...
              <div className="mb-6">
                <div className="bg-slate-700/50 rounded-xl p-4 inline-flex items-center gap-3">
                  {(current.step.includes("Opening Takbīr") || current.step.includes("Al-Fātiḥa") || current.step.includes("Additional Surah") || current.step.includes("Rukū'") || current.step.includes("Rising from Rukū'") || current.step.includes("Sujūd") || current.step.includes("Jalsa") || current.step.includes("Tashahhud") || current.step.includes("Salawat") || current.step.includes("Tasleem")) ? (
                    ...
                    <>
                      <button className="p-2 rounded-full bg-purple-400/20 hover:bg-purple-400/30 transition-colors opacity-50 cursor-not-allowed">
                        <Play className="w-5 h-5 text-purple-400" />
                      </button>
            ...
            <div className="flex gap-4">
              ...
              <button
                onClick={handleNext}
                className="flex-1 px-6 py-3 bg-gradient-to-r from-purple-500 via-pink-600 to-rose-700 hover:from-purple-400 hover:via-pink-500 hover:to-rose-600 text-white font-semibold rounded-xl transition-all duration-300 hover:scale-105 shadow-lg"
              >
                {currentStep === MAGHRIB_STEPS.length - 1
                  ? "Complete Prayer"
                  : "Next Step"}
              </button>
            </div>
